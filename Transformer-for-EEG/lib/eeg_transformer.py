...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        ...
        return self.generator(self.decode(self.encode(torch.transpose(src,0,1), src_mask), src_mask,
                            tgt, tgt_mask))
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout, max_len=10000):
        ...
        test = torch.arange(0.0, d_model - 1, 2)
        test2 = -(math.log(10000.0) / d_model)
        div_term = torch.exp(test * test2)
...
def make_model(src_d,tgt_d,N=6, d_model=512, d_ff=2048, h=7, dropout=0.1):
